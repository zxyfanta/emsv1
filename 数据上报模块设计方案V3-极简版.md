...
> 本文档中的**GPS优先级配置**已被移除！
>
> 系统现在根据设备上报的`useful`字段自动选择最优GPS（北斗或基站），无需手动配置。
>
> 详见：[GPS数据处理优化方案.md](./GPS数据处理优化方案.md)
>
> 变更摘要：
> - ❌ 删除：`gpsPriority`配置字段（BDS/LBS/BDS_THEN_LBS）
> - ✅ 新增：GPS自动选择逻辑（根据`useful`字段）
> - ✅ 简化：数据库Schema，统一GPS存储格式
> - ✅ 优化：上报逻辑，直接使用选中的GPS数据
...
### 固定不变的配置
- ✅ 上报地址: `http://59.225.208.12:18085/access/data/report`
- ✅ API Key: `AK-9lKIK7RZrHxPsxq9eYzb2BWBCq8WeSwIPC2e`
- ✅ 公钥获取地址: `http://59.225.208.12:18085/system/openApi/getPublicKey`
- ✅ 数据格式: 固定的JSON格式
- ✅ 加密方式: SM2(固定)

### 真正需要配置的只有3项:
1. **是否启用上报** - 开关
2. **上报粒度** - MIN/HOUR/DAY (影响 `dataType` 字段)
3. **GPS优先级** - BDS/LBS/BDS_THEN_LBS (影响 `LNG`/`LAT` 取值)
...
ADD COLUMN data_report_enabled BOOLEAN DEFAULT FALSE COMMENT '是否启用数据上报',
ADD COLUMN data_report_granularity VARCHAR(20) DEFAULT 'HOUR' COMMENT '上报粒度: MIN/HOUR/DAY',
ADD COLUMN data_report_gps_priority VARCHAR(20) DEFAULT 'BDS' COMMENT 'GPS优先级: BDS/LBS/BDS_THEN_LBS',
ADD COLUMN data_report_last_time DATETIME COMMENT '最后上报时间',
ADD COLUMN data_report_total_count INT DEFAULT 0 COMMENT '总上报次数',
ADD COLUMN data_report_success_count INT DEFAULT 0 COMMENT '成功次数',
ADD COLUMN data_report_failure_count INT DEFAULT 0 COMMENT '失败次数';
```
...
public enum ReportGranularity {
    MIN,    // 分钟级 (dataType: "MIN")
    HOUR,   // 小时级 (dataType: "HOUR")
    DAY     // 天级 (dataType: "DAY")
}
...
public class DataReportService {
    ...
    public void scheduledReport() {
        ...
        for (Company company : companies) {
            try {
                ...
                log.error("企业 {} 数据上报失败", company.getCompanyName(), e);
            }
        }
    }
    ...
    public void reportCompanyData(Company company) {
        ...
        try {
            ...
            if (dataList.isEmpty()) {
                log.warn("企业 {} 没有需要上报的数据", company.getCompanyName());
                return;
            }
            ...
            for (Map.Entry<String, List<RadiationDeviceData>> entry : groupedByDevice.entrySet()) {
                try {
                    ...
                    log.error("设备 {} 上报失败", entry.getKey(), e);
                    failureCount++;
                }
            }
    ...
    private DataItem convertToDataItem(RadiationDeviceData data) {
        ...
        dataMap.put("Nuclide", data.getDevice().getDescription() != null ?
            data.getDevice().getDescription() : "未知");
    ...
    private LocalDateTime calculateDataStartTime(Company company) {
        ...
        if (lastReport == null) {
            ...
            switch (company.getDataReportGranularity()) {
                case MIN:
                    return LocalDateTime.now().minusMinutes(10);  // 取最近10分钟
                case HOUR:
                    return LocalDateTime.now().minusHours(1);    // 取最近1小时
                case DAY:
                    return LocalDateTime.now().minusDays(1);     // 取最近1天
                default:
...
app:
  ems:
    features:
      data-report-enabled: ${EMS_FEATURE_DATA_REPORT:true}  # 全局开关
    ...
    data-report:
      ...
      sm2:
        enabled: true
        public-key-cache-duration: 3600000  # 1小时
  ...
  <div class="data-report-config">
    <el-card>
      ...
      <el-table :data="companies" v-loading="loading">
        ...
        <el-table-column label="上报粒度" width="150">
          <template #default="{ row }">
            <el-select
            ...
            >
              <el-option label="分钟级" value="MIN" />
              <el-option label="小时级" value="HOUR" />
              <el-option label="天级" value="DAY" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="GPS优先级" width="180">
          <template #default="{ row }">
            <el-select
              ...
              :disabled="!row.dataReportEnabled"
            >
              <el-option label="北斗优先" value="BDS" />
              <el-option label="基站优先" value="LBS" />
              <el-option label="北斗然后基站" value="BDS_THEN_LBS" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column prop="dataReportLastTime" label="最后上报" width="160">
          <template #default="{ row }">
            {{ row.dataReportLastTime || '未上报' }}
          </template>
        </el-table-column>
  ...
  <div class="my-report-status">
    <el-card>
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="上报粒度">
          {{ getGranularityLabel(config.granularity) }}
        </el-descriptions-item>
        <el-descriptions-item label="GPS优先级">
          {{ getGpsPriorityLabel(config.gpsPriority) }}
        </el-descriptions-item>
        <el-descriptions-item label="最后上报">
          {{ config.lastReportTime || '从未上报' }}
        </el-descriptions-item>
        <el-descriptions-item label="上报次数">
          {{ config.totalCount || 0 }} 次
        </el-descriptions-item>
